...
def encrypt(msg, key, decrypt=False):
    ...
    key = permutationByTable(key, 64, PC1)  # 64bit -> PC1 -> 56bit

    # split up key in two halves
    # generate the 16 round keys
    C0 = key >> 28
    D0 = key & (2 ** 28 - 1)
    roundKeys = generateRoundKeys(C0, D0)  # 56bit -> PC2 -> 48bit

    msgBlock = permutationByTable(msg, 64, IP)
    L0 = msgBlock >> 32
    R0 = msgBlock & (2 ** 32 - 1)
    ...
    for i in range(1, 17):
        if decrypt:  # just use the round keys in reversed order
            i = 17 - i
        L_round = R_last
        R_round = L_last ^ roundFunction(R_last, roundKeys[i])
        L_last = L_round
        R_last = R_round

    # concatenate reversed
    cipherBlock = (R_round << 32) + L_round

    # final permutation
    cipherBlock = permutationByTable(cipherBlock, 64, IP_INV)

    return cipherBlock


def roundFunction(Ri, Ki):
    # expand Ri from 32 to 48 bit using table E
    Ri = permutationByTable(Ri, 32, E)

    # xor with round key
    Ri ^= Ki

    # split Ri into 8 groups of 6 bit
    RiBlocks = [((Ri & (0b111111 << shift_val)) >> shift_val) for shift_val in (42, 36, 30, 24, 18, 12, 6, 0)]

    # interpret each block as address for the S-boxes
    for i, block in enumerate(RiBlocks):
        ...
        RiBlocks[i] = Sboxes[i][16 * row + col]

    # pack the blocks together again by concatenating
    RiBlocks = zip(RiBlocks, (28, 24, 20, 16, 12, 8, 4, 0))
    Ri = 0
    for block, lshift_val in RiBlocks:
        Ri += (block << lshift_val)

    # another permutation 32bit -> 32bit
    Ri = permutationByTable(Ri, 32, P)

    return Ri


def permutationByTable(block, block_len, table):
    # quick and dirty casting to str
    blockStr = bin(block)[2:].zfill(block_len)
    perm = []
    for pos in range(len(table)):
        perm.append(blockStr[table[pos] - 1])
    return int(''.join(perm), 2)


def generateRoundKeys(C0, D0):
    # returns dict of 16 keys (one for each round)

    roundKeys = dict.fromkeys(range(0, 17))
    lrotValues = (1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1)
    ...
    for i, rot_val in enumerate(lrotValues):
        i += 1
        Ci = lrot(roundKeys[i - 1][0], rot_val, 28)
        Di = lrot(roundKeys[i - 1][1], rot_val, 28)
        roundKeys[i] = (Ci, Di)

    # roundKeys[1] for first round
    #           [16] for 16th round
    # dont need roundKeys[0] anymore, remove
    del roundKeys[0]

    # now form the keys from concatenated CiDi 1<=i<=16 and by apllying PC2
    for i, (Ci, Di) in roundKeys.items():
        Ki = (Ci << 28) + Di
        roundKeys[i] = permutationByTable(Ki, 56, PC2)  # 56bit -> 48bit
